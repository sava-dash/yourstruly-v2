-- ============================================================================
-- FINAL FIX V3: Correct column order (life_chapter goes LAST)
-- ============================================================================
...
RETURNS TABLE (
  ...
  life_chapter TEXT  -- MOVED TO END (column 32)
) AS $$
DECLARE
...
BEGIN
  ...
  SELECT 
    r.id, r.user_id, r.type, r.category,
    ...
    r.metadata, r.updated_at, r.life_chapter  -- LIFE_CHAPTER LAST
  FROM ranked r
  WHERE r.type_rank <= 2
...
SELECT 'Column order fixed!' as status;
