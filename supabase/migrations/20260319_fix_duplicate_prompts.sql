-- Fix: filter out prompts whose text matches any previously answered prompt
...
DECLARE
  ...
  v_answered_texts TEXT[];
BEGIN
  -- Get ALL previously answered prompt texts to never repeat them
  SELECT ARRAY_AGG(DISTINCT prompt_text) INTO v_answered_texts
  FROM engagement_prompts
  WHERE user_id = p_user_id
    AND status IN ('answered', 'skipped');
  
  -- Check how many pending prompts exist (excluding already-answered texts)
  SELECT COUNT(*) INTO v_pending_count
  FROM engagement_prompts
  WHERE user_id = p_user_id
    AND status = 'pending'
    AND (cooldown_until IS NULL OR cooldown_until < NOW())
    AND (v_answered_texts IS NULL OR prompt_text != ALL(v_answered_texts));
  ...
  available AS (
    ...
    WHERE ep.user_id = p_user_id
      ...
      AND (v_answered_texts IS NULL OR ep.prompt_text != ALL(v_answered_texts))
  ),
  photo_pick AS (
  ...
  fill_picks AS (
    ...
    WHERE cat_rank = 1
    ORDER BY 
      CASE WHEN source = 'profile_based' THEN 0 ELSE 1 END,
      priority DESC,
      RANDOM()
