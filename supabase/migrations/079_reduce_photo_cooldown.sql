-- Migration 079: Reduce photo prompt cooldown from 30 to 10 days
-- Also fix file_type filter to handle both 'image' and 'image/%' formats
...
BEGIN
  ...
  -- Cooldown reduced to 10 days (was 30)
  -- ============================================
  FOR v_photo IN 
    ...
    WHERE mm.user_id = p_user_id
      ...
      -- Exclude photos that already have recent prompts (10 day cooldown)
      AND NOT EXISTS (
  ...
  -- ============================================
  -- GENERIC PROMPTS (fill remainder to ensure we always have prompts)
  -- ============================================
  FOR v_template IN 
    SELECT * FROM prompt_templates 
    WHERE is_active = TRUE 
    ...
    ORDER BY RANDOM()
    LIMIT GREATEST(5, p_count - v_prompt_count)  -- Always generate at least 5
  LOOP
...
COMMENT ON FUNCTION generate_engagement_prompts IS 'Generate personalized prompts. Photo cooldown: 10 days. Always generates at least 5 generic prompts as fallback.';
