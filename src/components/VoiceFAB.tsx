...
const ACTIONS = [
  { icon: Mic, label: 'Voice Memory', href: '/dashboard/memories/voice', color: 'bg-[#2D5A3D]' },
  ...
  return (
    <div className="fixed bottom-24 right-4 lg:bottom-8 lg:right-8 z-40 flex flex-col items-end gap-2">
      ...
      <button
        ...
        aria-expanded={open}
        className={`flex h-14 w-14 items-center justify-center rounded-full shadow-lg transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2D5A3D]/60 focus-visible:ring-offset-2 ${
          open
            ? 'bg-neutral-700 rotate-45 hover:bg-neutral-600'
            : 'bg-[#2D5A3D] hover:bg-[#234A31] shadow-[#2D5A3D]/20'
        }`}
      >
